...
from tradingagents.agents.reviewers import run_ai_report_review
...
class MessageBuffer:
    ...
    ANALYST_MAPPING = {
        ...
        "industry_chain": "Industry Chain Analyst",
    }
    ...
    REPORT_SECTIONS = {
        ...
        "annual_report": ("annual_report", "Annual Report Analyst"),
        "industry_report": ("industry_chain", "Industry Chain Analyst"),
    ...
    def _update_current_report(self):
        ...
        if latest_section and latest_content:
            ...
            section_titles = {
                ...
                "industry_report": "Industry Chain Analysis",
    ...
    def _update_final_report(self):
        ...
        analyst_sections = ["market_report", "sentiment_report", "news_report", "fundamentals_report", "annual_report", "industry_report"]
        if any(self.report_sections.get(section) for section in analyst_sections):
            ...
            if self.report_sections.get("annual_report"):
                report_parts.append(
                    f"### Annual Report Analysis\n{self.report_sections['annual_report']}"
                )
            if self.report_sections.get("industry_report"):
                report_parts.append(
                    f"### Industry Chain Analysis\n{self.report_sections['industry_report']}"
                )
...
def get_user_selections():
    ...
    # Step 0: Wind API key for portable A-share setup
    ensure_wind_api_key()
    ...
    console.print(
        create_question_box(
            "Step 1: Ticker Symbol",
            "Enter the A-share Wind code with exchange suffix (e.g. 600519.SH, 000001.SZ)",
            "600519.SH",
        )
    )
...
def display_complete_report(final_state):
    ...
    if final_state.get("annual_report"):
        analysts.append(("Annual Report Analyst", final_state["annual_report"]))
    if final_state.get("industry_report"):
        analysts.append(("Industry Chain Analyst", final_state["industry_report"]))
    if analysts:
...
ANALYST_ORDER = ["market", "social", "news", "fundamentals", "annual_report", "industry_chain"]
ANALYST_AGENT_NAMES = {
    "market": "Market Analyst",
    "social": "Sentiment Analyst",
    "news": "News Analyst",
...
ANALYST_REPORT_MAP = {
    ...
    "industry_chain": "industry_report",
}
...
def run_analysis(checkpoint: bool | None = None):
    ...
    # Run a lightweight post-report reviewer before saving. This is best-effort:
    # a review failure should never prevent the completed report from being saved.
    if os.environ.get("TRADINGAGENTS_DISABLE_AI_REVIEW", "").lower() not in {"1", "true", "yes"}:
        try:
            console.print("[cyan]Running AI report review...[/cyan]")
            final_state["ai_quality_review"] = run_ai_report_review(
            ...
            final_state["ai_quality_review"] = (
                "# AI Report Review\n\n"
                "REVIEW_UNAVAILABLE: The post-report AI reviewer failed, but the report was saved.\n\n"
                f"Error: {e}"
            )

    # Auto-save to the default report directory. This avoids losing a completed
    # run when the interactive save-path prompt is cancelled or mis-entered.
    ...
    try:
        ...
        if final_state.get("ai_quality_review"):
            console.print("  [dim]AI review:[/dim] 6_quality/ai_review.md")
    except Exception as e:
