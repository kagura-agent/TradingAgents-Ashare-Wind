...
ANALYST_ORDER = [
    ...
    ("Annual Report Analyst", AnalystType.ANNUAL_REPORT),
...
def is_valid_ticker_input(value: str) -> bool:
    ...
    if not v:
        return True
    if not (all(ch.isalnum() or ch in "._-^=" for ch in v) and len(v) <= 32):
        return False
...
def ticker_validation_message(value: str) -> str:
    v = value.strip().upper()
    if re.fullmatch(r"\d{1,5}\.(SH|SZ|BJ)", v):
        return "A-share Wind codes must use 6 digits, e.g. 002463.SZ, 600519.SH."
    return "Please enter a valid ticker symbol, e.g. 002463.SZ, 600519.SH, 300750.SZ."
...
def get_ticker() -> str:
    ...
    ticker = questionary.text(
        ...
        validate=lambda x: (
            ...
            or ticker_validation_message(x)
        ),
...
def filter_analysts_for_asset_type(
    ...
    return [
        ...
        if analyst not in (AnalystType.FUNDAMENTALS, AnalystType.ANNUAL_REPORT, AnalystType.INDUSTRY_CHAIN)
    ]
...
def _llm_provider_table() -> list[tuple[str, str, str | None]]:
    """(display_name, provider_key, base_url) for A-share build providers.

    The A-share Wind build keeps the interactive picker focused on domestic or
    user-controlled endpoints to avoid accidental calls to overseas providers.
    ...
    return [
        ...
        ("OpenAI-compatible (local or user-controlled endpoint)", "openai_compatible", None),
    ]
...
def ask_glm_region() -> tuple[str, str]:
    """Use the China GLM endpoint in the A-share build."""
    return ("glm-cn", "https://open.bigmodel.cn/api/paas/v4/")


def ask_qwen_region() -> tuple[str, str]:
    """Use the China DashScope endpoint in the A-share build."""
    return ("qwen-cn", "https://dashscope.aliyuncs.com/compatible-mode/v1")


def ask_minimax_region() -> tuple[str, str]:
    """Use the China MiniMax endpoint in the A-share build."""
    return ("minimax-cn", "https://api.minimaxi.com/v1")
...
def ensure_wind_api_key() -> str | None:
    """Ensure a project-local WIND_API_KEY exists for Wind MCP access.

    Wind's CLI also supports user-global and skill-local key storage. This
    helper is a sharing-friendly project path: if the current process or the
    project's .env already has WIND_API_KEY, it does nothing; otherwise it
    prompts once and persists to the local .env.
    """
    env_var = "WIND_API_KEY"
    existing = os.environ.get(env_var)
    if existing:
        return existing
    if os.environ.get("TRADINGAGENTS_SKIP_WIND_KEY_SETUP", "").lower() in {"1", "true", "yes"}:
        return None

    console.print(
        "\n[yellow]WIND_API_KEY is not set for this project.[/yellow]\n"
        "[dim]Wind may also work if you configured the key globally in wind-mcp-skill; "
        "for a portable project setup, paste your Wind key here.[/dim]"
    )
    key = questionary.password(
        "Paste your WIND_API_KEY (will be saved to this project's .env; press Enter to skip):",
        style=questionary.Style([
            ("text", "fg:cyan"),
            ("highlighted", "noinherit"),
        ]),
    ...
    if not key:
        env_path = find_dotenv(usecwd=True) or str(Path.cwd() / ".env")
        Path(env_path).touch(exist_ok=True)
        set_key(env_path, "TRADINGAGENTS_SKIP_WIND_KEY_SETUP", "true")
        os.environ["TRADINGAGENTS_SKIP_WIND_KEY_SETUP"] = "true"
        console.print(
            "[yellow]Skipped WIND_API_KEY setup. Wind calls will rely on global/skill config, "
            "or fail if no key is configured.[/yellow]"
        )
        return None
