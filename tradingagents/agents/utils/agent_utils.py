...
def _parse_wind_basicinfo(text: str, ticker: str) -> dict[str, str]:
    try:
    ...
    for table in tables:
        ...
        for row in table.get("rows", []):
            ...
            if wind_code and wind_code.upper() != ticker.upper():
                continue
            identity = {}
            short_name = _clean_identity_value(values.get("证券简称"))
            company_name = _clean_identity_value(values.get("公司中文名称"))
            exchange = _clean_identity_value(values.get("交易所中文名称"))
            industry = _clean_identity_value(values.get("WIND行业级别明细"))
            former_name = _clean_identity_value(values.get("公司曾用名"))
            if short_name:
            ...
            if former_name:
                identity["former_name"] = former_name
            if identity:
                return identity
    return {}
...
def resolve_instrument_identity(ticker: str) -> dict:
    ...
    if get_config().get("data_vendors", {}).get("core_stock_apis") == "wind":
        try:
            from tradingagents.dataflows.wind import _wind_nl

            text = _wind_nl("get_stock_basicinfo", f"{ticker} 公司基本档案、中文简称、所属行业、上市交易所")
            identity = _parse_wind_basicinfo(text, ticker)
            if identity:
                identity["sector"] = "A-share"
                return identity
            return {
                "company_name": f"Wind basicinfo unavailable for {ticker}; use ticker only",
                "sector": "A-share",
                "industry": "Wind basicinfo unavailable",
                "exchange": ticker.rsplit(".", 1)[-1] if "." in ticker else "A-share",
            }
        except Exception as exc:  # noqa: BLE001 — fail open, never block the run
            logger.debug("Could not resolve Wind instrument identity for %s: %s", ticker, exc)
            return {}
...
def build_instrument_context(
    ...
    if identity:
        ...
        if short_name:
            details.append(f"Short name: {short_name}")
        if name:
