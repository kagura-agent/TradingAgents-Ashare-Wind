...
class TradingAgentsGraph:
    ...
    def _fetch_returns(
        ...
        try:
            ...
            if self.config.get("data_vendors", {}).get("core_stock_apis") == "wind":
                import pandas as pd
                ...
                start_ts = pd.to_datetime(trade_date)
                stock = stock[pd.to_datetime(stock["Date"]) >= start_ts].sort_values("Date")
                bench = bench[pd.to_datetime(bench["Date"]) >= start_ts].sort_values("Date")
