...
describe('Timeline', () => {
  ...
  it('reflects node status transitions', () => {
    ...
    const { rerender } = render(<Timeline nodes={nodes} />)
    ...
    rerender(<Timeline nodes={{ 'Market Analyst': 'done', 'News Analyst': 'running' }} />)
  ...
  it('announces status in text for screen readers', () => {
    render(<Timeline nodes={{ Trader: 'running' }} />)
  ...
  it('ignores nodes it does not know about', () => {
    render(<Timeline nodes={{ 'Some Future Agent': 'running' }} />)
